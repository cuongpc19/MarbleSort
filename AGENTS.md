...
2. **Chute** — straight sides down from the grid, then a short cone into a neck barely wider
   ...
   ⚠ **The chute is frictionless — marbles and walls at friction 0 — and that is a finding, not a
   tuning (2026-09-07).** It was 0.008 / 0.02, "near-frictionless", and a lone marble set down on
   the bowl wall crept the whole bottom at 1-2 px/step, 1.7 s from mouth to throat, gaining 0.013
   px/step² where the slope owed ~0.08. Matter's resolver caches and accumulates the friction
   impulse on any contact slower than `_restingThreshTangent` (√6 ≈ 2.45 px/step) until it balances
   gravity, so *any* friction becomes an equilibrium crawl — and the same path glues two marbles
   touching with little relative motion, reported as *"các bi lăn trên phễu hơi dính vào nhau"*.
   Gravity in Matter's units is **0.29 px/step²** (1.05 × 0.001 × 16.67²), not 1.05, which is why a
   13° bowl bottom could never be quick. Measured over a nine-marble pour (level 5, cell 23): mean
   bowl speed 1.9 → 3.2 px/step, time parked 7-11% → 3-4%, marble-on-marble contact 0.44-0.88 → 0.26-0.37
   neighbours, last marble on the rail 3.1-4.0 s → 2.5-2.9 s — the floor now is the one-per-tick
   cadence itself. Three constants moved with it and belong together: the bowl's control point
   `CB.b` 0.49 → **0.30** (throat 13° → 17.7°, nothing under 15°), restitution 0.6 → **0.7** ("k có
   độ đàn hồi"), and `SWALLOW_SPEED` 3 → **6**, because frictionless arrivals reach the throat at
   ~4 px/step and the old 3 turned them away to swing for another second. The notes on
   `GameScene.dropMarble` and `SWALLOW_SPEED` carry the traces. The instrument is
   `npm run shot -- --level 5 --trace 23` (per-step trace of a pour; `--trace drop:x,y` sets one
   marble down on the wall, no tray, no neighbours) — use it before touching anything in the chute.
...
## Two beats of feel: the pour pops, the level fades in

- **The pop.** On the tap the nine marbles come loose at `POP_SCALE` = 1.25 and are back to size as
  they fall — "scale out ra màn hình rồi lại scale in (học bản game gốc)". Measured off the two
  Sep-5 clips: ring-fit radius 13.0 in the lifted pile against 11.0-11.5 falling, i.e. ~1.2x, held
  ~7-8 frames, back in 3-4; it is the reference's perspective camera (the pile lifts toward the
  lens). ⚠ Sprite only, never the body, and clamped by height as well as time — the scale is back
  to 1 by the mouth of the chute, so a bottom-row pour cannot push oversize sprites into the bowl
  or hand one to the rail mid-pop. `npm run shot -- --level 5 --taps 1 --after 160` catches it.
- **The fade.** `fadeInGrid`: the pieces (trays, crates, hatches, clips, badges, fixtures) fade up
  over 220ms, bottom row first, rows 28ms apart — "fade in nhẹ". The cavity and its slot plates do
  not; they are the machine. ⚠ The four intro cards (walkthrough, SUPER HARD warning, magnet lesson,
  coach) wait for it, together, so their order is untouched — a hand pointing at a tray that is not
  there yet points at nothing. Not on a preview. `--fadeprobe` reads the alphas frame by frame.
...
## Loop Sort teardown — `Manythings/LoopSort-teardown/`

Mổ APK của **Loop Sort** (Garawell/Voodoo 3.4.1), game cùng thể loại: cube chạy trên vòng ray
kín, xe theo màu đỗ vào bến hút cube. Level của họ là **JSON thuần** trong `data.unity3d` —
1299 level, 800 bố cục hàng đợi xe (`Carriers.json`), 800 đường ray (`Splines.json`), cộng
mốc mở cơ chế và hằng số kinh tế. Đã đối chiếu với clip gameplay lv1–5 và khớp 5/5, kể cả
`Splines.Closed=false` ứng đúng với Portal mở ở lv4.

Đọc `Manythings/LoopSort-teardown/README.md` trước khi cân lại đường cong độ khó — nó có
sẵn phân bố lane / số xe / màu / theme trên cả 1299 level của một game đã ship.
⚠ Tài liệu tham khảo để học cách họ dựng độ khó, **không copy level vào build**.

### Bản chơi thử — `tools/loopsort/`

`loopsort.js` là engine (vật lý + vẽ), `app.js` là vỏ game (Home, luồng level, ví, booster),
`headless.mjs` là thước đo không giao diện. Mở bằng `/tools/loopsort/index.html` trên dev
server; `?level=N`, `?dev=1`, và `window.__ls` là cửa cho `scripts/shot.mjs --then`.

Giao diện bám theo **`Game/UI_UX_RULES.md`** (thư mục cha) — luật Home/HUD rút ra từ Tube
Tangle, dùng chung cho mọi game casual portrait trong thư mục đó. Số hiệu luật được chép ngay
vào comment tại chỗ áp dụng, nên sửa chỗ nào thì đọc luật ở đó trước.

⚠ **Hai dải HUD là DOM, bàn cờ là canvas, nên canvas phải được *đo* chứ không được đoán.**
`setChrome(headCss, footCss)` nhận chiều cao thật của `#topbar` và `.tools`; trước đây nó là
một cờ bool và `layout()` đoán 9.2%/8.8% chiều cao canvas — đúng trên máy này và sai trên máy
có safe-area (luật 2.7), tức bàn cờ chui xuống dưới gầm hàng booster, đúng cái lỗi mà `foot`
sinh ra để chặn.

⚠ **Ô đếm cube ở lại canvas, số level chuyển hẳn sang DOM.** Thanh trên tối đa 4 vật thể
(luật 2.1) và đã đủ bốn: về nhà · chơi lại · thẻ level · ví. Ô đếm là đồng hồ của *bàn cờ* —
nó đổi sang đỏ theo ray và phóng to nhỏ theo bàn cờ — nên nó nằm ngay dưới dải HUD chứ không
tranh chỗ trong đó; ở 360px thì trong dải cũng không còn chỗ. Ngược lại, nhãn độ khó (luật
2.8) và vùng chạm 44px (2.6) là việc của DOM, nên số level rời canvas.

⚠ **`Game.canUndo()` tồn tại để nút booster mờ được *trước* khi bấm** (luật 3.5), và nó là một
định nghĩa duy nhất — `undo()` gọi chính nó. Chép điều kiện thành bản thứ hai thì hai bản sẽ
trôi khỏi nhau, và nút sáng trong khi engine từ chối.

⚠ **Hết lượt booster thì HỎI trước khi trừ tiền.** Bản đầu trừ thẳng 300 xu ngay trên cú chạm
đầu tiên, không một lời nào — giá có hiện sẵn trên icon thật (luật 2.5), nhưng chạm vào giá
lại thành đồng ý mua. Thẻ `MUA VÀ DÙNG` mua xong dùng luôn, và `Thôi` cùng bề ngang (3.8).

⚠ **Thẻ RAY TẮC chỉ đường bằng chính artwork của thanh booster** (3.3 + 3.4), và hai nút đó
bấm được, làm luôn việc nó nói. Trước đó lớp phủ của thẻ nuốt mọi cú chạm nên thanh booster
dưới gầm không với tới được — `onBooster` đã có sẵn nhánh gỡ thẻ khi cứu được bàn, chỉ là
không có đường nào bấm tới nó. `Ray +1` tự mờ khi một chỗ không đủ cứu (3.5).

⚠ **Hết chỗ trên ray KHÔNG phải là thua — chỉ là không chạm được vali nữa.** Trước đây tràn
ray là `state = "lose"` ngay lập tức, và nó sai cả về luật lẫn về cảm giác: ray đầy là một
trạng thái **tạm thời**, nó tự gỡ khi hàng chạy vào bến, còn thua thì không gỡ được. Giờ
`Game.canTap(t)` là cửa duy nhất — `tap()` gọi chính nó, nên cái hỏi trước và cái đổ ra không
thể lệch nhau — và nó hỏi `counter() + tapLoad(t) <= slotCount`. ⚠ Phải cộng `tapLoad`: một
vali đổ ra cả đoạn cùng màu một lúc, nên nó có thể bị khoá trong khi ray vẫn còn một chỗ trống.
Thua chỉ còn đúng một nghĩa: `isStuck()` — không vali nào chạm được **và** không miếng hàng nào
trên ray có bến nhận. ⚠ `isStuck()` chỉ hỏi khi mọi thứ đã **đứng yên** (không còn `pending`,
`flying`, bến nào đang rút hàng hay đang hút); hỏi giữa lúc hàng đang bay là đoán mò.

⚠ **Thẻ RAY TẮC khi ray khoá, và nó là một thẻ BÁO chứ không phải thẻ THUA.** `jammed(g)` =
còn vali có hàng mà không vali nào chạm được. Ba thứ giữ cho nó không thành phiền, và cả ba
đều là luật trong `Game/UI_UX_RULES.md`:
- **Một lần một trạng thái, và cờ có chỗ gỡ** (3.1): `jamTold` bật khi khoá, tắt ngay khi có
  vali chạm được trở lại — và lúc đó thẻ **tự đóng**. Thẻ báo một trạng thái, trạng thái hết
  thì thẻ phải hết theo; để nó nằm lại là che mất đúng cái bàn cờ mà nó vừa bảo người chơi nhìn.
- ⚠ **Điều kiện tắc KHÔNG phải "ray đầy", mà là "mọi miếng hàng đã đi tròn một vòng ray mà
  vẫn không bến nào nhận"** (`c.lap >= g.len`, cộng với không vali nào chạm được và không còn
  hàng đang tuôn ra hay đang bay vào xe). Bản đầu bật ngay khi không chạm được vali nào, và nó
  **sớm hẳn một vòng ray**: lúc đó hàng vẫn đang chạy tới bến của nó, tức là thẻ báo một chuyện
  chưa xảy ra. Đo trên level 9: khoá lúc t=0, min lap bò 0.11 → 0.88 suốt 6 giây, tới 1.03 ở
  giây thứ 7 thì thẻ mới bật. Một vòng ray hết vài giây, nên chính nó là khoảng chờ — không cần
  hẹn giờ riêng nào nữa (`JAM_HOLD_MS` đã bỏ).
- ⚠ `c.lap` là **một** bộ đếm quãng đường dùng cho cả hai việc: dưới `FREE_RUN` thì miếng hàng
  vẫn là hàng của bến vừa đổ (chặn tự nuốt), quá `len` thì nó đã chào hết mọi bến. Trước đây
  `c.run` chỉ cộng khi còn `src` nên không đo được vòng; gộp làm một để hai con số không trôi
  khỏi nhau.
- **Vẽ bàn cờ thật** (3.7): dải trên thẻ mang đúng màu đang nằm trên ray lúc đó, dồn vào một
  cái chặn. Cách gỡ là đúng nút `Ray +1` của thanh booster (3.3 + 3.4), bấm được, và tự mờ khi
  một chỗ không đủ cứu (3.5).

⚠ **`@keyframes pop` chỉ động `transform`, không động `opacity`.** Một thẻ đang hiện mà trong
suốt ở khung hình đầu tiên thì khi trình duyệt nghẽn — WebGL biên dịch shader đúng lúc thẻ bật
ra là trường hợp có thật ở đây — nó đứng yên ở 0%, tức một thẻ **vô hình che kín màn hình**.
Đã bắt được đúng cảnh đó khi chụp màn hình: thẻ có thật, đúng chỗ, `opacity` 0, animation vẫn
"running" sau 600ms của một hoạt hình dài 260ms. Phóng to thì hỏng cũng chỉ là không nảy.

⚠ **Bộ vẽ 2D trong `loopsort.js` đã chết** — `app.js` không gọi `E.draw` nữa, `three3d.js` vẽ
tất cả. `drawHud`/`drawBay`/`drawBlocked` vẫn còn trong file nhưng không có gì trên màn hình đi
qua chúng; ô đếm giờ là `#beltGauge` bằng DOM. Sửa hình thì sửa bên `three3d.js`.
...
| màu | 5.4 | 7.8 | 9.0 | 9.8 | 10.1 | 11.3 | 11.3 | 11.7 | 11.5 |
| xe | 22 | 31 | 36 | 39 | 40 | 45 | 45 | 47 | 46 |
| slot | 8.5 | 7.2 | 6.2 | 6.7 | 6.4 | 5.9 | 6.2 | 6.0 | 5.5 |

⚠ **`xe = 4 × số màu` không phải trùng hợp mà là SỐ HỌC của trò chơi.** `DELIVER` = 4, nên
một màu không chia hết cho 4 là một màu không bao giờ giao xong, tức bàn cờ không thể thắng.
Kiểm lại: **1299/1299 level của họ đều chia hết**. Bộ sinh phát 4 miếng mỗi màu, không có
đường nào khác.

⚠ **Số lane phải bám theo số màu, không nội suy độc lập.** Họ luôn để lane nhiều hơn màu một
chút (11.9 lane / 11.5 màu) — tức **~4 miếng một lane, vừa đúng một khoang xe**. Bản đầu nội
suy hai đường riêng, có chỗ lane < màu, mỗi lane hơn 4 miếng, và khi đó **bến nào cũng đầy nên
bến nào cũng từ chối nhận** — bàn chết ngay từ nước đầu. Level 30 hỏng cả 6 lần gieo lại trước
khi tìm ra nguyên nhân này.

**Hình ray**: đa giác vuông góc trên lưới số nguyên, đúng cách họ dựng (`SHAPES` / `OPEN_SHAPES`
— chữ nhật, khuyết, L, T, zigzag, thắt eo, chữ thập, bậc thang; ray hở cho Portal). Cạnh có bến
đỗ phải dài ≥ 6 vì góc bo tròn ăn mất 2.2 mỗi đầu.

⚠ **Bến đỗ nằm NGOÀI vòng ray, và phép đo va chạm làm trên CẢ THÂN XE.** Thân xe dài
`4 × SLOT_LEN` = 6.7 đv: một bến đặt đúng khoảng cách vẫn có thể chạm thân sang một khúc ray
khác nằm sau nó, và trên màn hình nó đọc ra là *cái xe nằm đè lên đường ray*. Hai bến ở hai
cạnh kề nhau cũng vậy — hai điểm đỗ cách nhau 4.2 đv là thoáng, nhưng hai cái thân thì cắt
nhau, và engine chỉ còn cách co cả đàn xe lại (`fit`): đo được `fit 0.12` ở một bản nháp, tức
bàn cờ vẫn chạy nhưng bé tí và không ai hiểu tại sao.

⚠ **`rot` của bến suy từ hướng tới ray, nên không bao giờ lệch.** Level 9 *của họ* có bến lệch
tới 36° so với pháp tuyến ray (dock đặt trên lưới đều, `rot` làm tròn về ±90°, trong khi ray ở
đó chạy chéo) — đó chính là cái "dốc nối xiên xẹo" bị báo. Bộ sinh của mình tính `rot` từ hướng
thật tới ray nên lỗi đó không tồn tại được; và đó cũng là một phép nghiệm thu: **mọi bến phải
lệch ≤ 12° và cách ray 1.8–4.6 đv**, không đạt thì gieo lại.

**Nghiệm thu**: `levelgen.mjs` kiểm HÌNH HỌC trước rồi mới cho bot chơi thật một ván bằng
**chính engine** (`levelbot.mjs`), level nào không thắng thì gieo lại, tối đa 6 vòng.
⚠ Hình học phải kiểm trước vì **một bàn cờ có xe chồng nhau vẫn thắng được** — bot không bao
giờ báo cho biết. ⚠ Và nghiệm thu bằng engine thật chứ không bằng một bản mô phỏng rút gọn: bản
rút gọn là bản sao thứ hai của luật chơi, hai bản sẽ trôi khỏi nhau, và lúc đó "đã kiểm tra"
không còn nghĩa gì.

**Con bot** (`levelbot.mjs`) — ba cái bẫy đã sập, ghi lại vì cái nào cũng làm nó thua sạch:
- ⚠ **Chờ theo ngưỡng tuyệt đối thì đứng hình.** "Chạm khi `counter <= slotCount/2`" nghe hợp
  lý và cho **0% thắng ở mọi level**: hàng không bến nào nhận nằm lì trên ray, counter không
  bao giờ xuống dưới ngưỡng, bot không chạm nữa, chạy hết 240 giây. Kiên nhẫn phải là "chờ ray
  VƠI ĐI" (`flat`), không phải chờ một con số.
- ⚠ **Phải LỌC rồi mới chấm điểm.** Viết kiểu "không bến nào nhận thì trừ 20 điểm" thì lúc mở
  màn mọi nước đều âm như nhau, bot vẫn chạm, và đổ ngay một màu không ai nhận ra ray. Đổ màu
  không ai nhận chỉ được phép khi ray sạch — lúc đó nó còn trọn một vòng để tìm bến vừa mở.
- ⚠ **Đừng đổ cái bến đang gom.** Bến "thuần màu" là bến đang gom, đổ nó ra là phá đúng việc nó
  đang làm. Thiếu luật này bot đánh bóng bàn vĩnh viễn: trên level 1 (bốn miếng đỏ, hai bến) nó
  đổ qua đổ lại **131 lần** mà không bên nào gom đủ bốn.

```
node tools/loopsort/levelgen.mjs              # sinh + nghiệm thu toàn bộ, ghi vào data/
node tools/loopsort/levelgen.mjs --to 40      # 40 level đầu, để xem hình
node tools/loopsort/levelbot.mjs 1-50         # đo độ khó một dải (bot thắng bao nhiêu %)
