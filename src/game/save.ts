// Progress + wallet. Small enough to keep in key-value storage; every key is prefixed `bs_`
...
// renaming `bs_*` after launch restores the old names into a game that reads the new ones and
// every player loses everything. Before launch it is free; after, it never is.
//
// ⚠ **It has moved twice, and the second move was not free — it was paid for.** `ms_` -> `bf_` on
// 2026-08-13 cost nothing because nobody had played. `bf_` -> `bs_` on 2026-09-05 was made with
// 3442 real players holding progress under `bf_`, measured from the play log, and every one of
// them loses it. That was the right call only because the build is going up as a **separate
// listing** — a new game with its own audience, not an update to the old one, so there is nobody
// to carry across. Do not read this as a precedent: for an update in place it would have been
// straightforwardly destructive, and the fix then is a one-time migration that reads the old keys
// and writes the new ones, not a rename.

import { FREE_MAGNETS } from "./config";
import { platform } from "../platform";

const K_LEVEL = "bs_level";
const K_STARS = "bs_stars";
const K_COINS = "bs_coins";
const K_MUTE = "bs_mute";
/** Has the player been walked through level 1 yet? */
const K_TUTOR = "bs_tutor";
/**
 ...
 * renaming an existing `bs_` key after launch restores the old name into a game reading the new
 * one and wipes the player out. Adding keys is free; renaming them is not.
 */
const K_COACH = "bs_coach";
/**
 ...
 * whatever it held.
 */
const K_DAILY = "bs_daily";
/**
 * The day the daily reward was last **offered** — not claimed. `bs_daily` records the claim.
 *
 ...
 * new one. This key was renamed once, from `bs_freemag`, before any build carrying it was uploaded
 ...
 */
const K_FREE_REVIVE = "bs_freerevive";
